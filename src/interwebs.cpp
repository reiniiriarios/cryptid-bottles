...
Interwebs::Interwebs(WiFiClient* client, const char* ssid, const char* wifi_pass,
...
void Interwebs::loop(void) {
  // if (this->status != INTERWEBS_STATUS_OKAY) {
  //   // Prints a lot.
  //   DEBUG_PRINT(F("Interwebs: "));
  ...
  switch (this->status) {
    ...
    case INTERWEBS_STATUS_MQTT_CONNECTED_TO_BROKER:
      ...
      return;
    case INTERWEBS_STATUS_MQTT_CONNECTION_CONFIRMED:
    case INTERWEBS_STATUS_MQTT_SUBSCRIPTION_FAIL:
    case INTERWEBS_STATUS_MQTT_SUBSCRIBING:
      this->mqttSubscribe();
      return;
    case INTERWEBS_STATUS_MQTT_SUBSCRIBED:
      this->mqttAnnounce();
      return;
    case INTERWEBS_STATUS_MQTT_ANNOUNCED:
    case INTERWEBS_STATUS_SENDING_DISCOVERY:
      this->sendDiscoveries();
      return;
    case INTERWEBS_STATUS_READING_CONACK_PACKET:
    case INTERWEBS_STATUS_READING_SUBACK_PACKET:
    case INTERWEBS_STATUS_READING_SUB_PACKET:
    case INTERWEBS_STATUS_READING_PING_PACKET:
    case INTERWEBS_STATUS_READING_PACKET:
      this->readFullPacketUntilComplete();
      return;
    case INTERWEBS_STATUS_SUBSCRIPTION_PACKET_READ:
      this->handleSubscriptionPacket(this->full_packet_len); // @todo remove param
      this->full_packet_len = 0; // @todo move this into method
      return;
    case INTERWEBS_STATUS_SUBSCRIPTION_IN_QUEUE:
      this->processSubscriptionQueue();
      return;
    default:
      Serial.print("Error: Unrecognized interwebs status: ");
      Serial.println(this->status);
    case INTERWEBS_STATUS_OKAY:
      // If something is available and there's no current status, it might be a subscription packet.
      if (this->wifiClient->available()) {
        this->status = INTERWEBS_STATUS_READING_SUB_PACKET;
        this->readFullPacketUntilComplete();
        return;
      }
      // Check connection.
...
bool Interwebs::mqttConnectBroker() {
  ...
  this->status = INTERWEBS_STATUS_READING_CONACK_PACKET;
  DEBUG_PRINT(F("reading conack.."));
  this->readFullPacketUntilComplete();
  return true;
}

bool Interwebs::confirmConnectToBroker() {
  this->attempts = 0;
  ...
  this->status = INTERWEBS_STATUS_MQTT_CONNECTION_CONFIRMED;
  return true;
}

bool Interwebs::mqttSubscribe(void) {
  // If already in this loop, move to the next item.
  if (this->status == INTERWEBS_STATUS_MQTT_SUBSCRIBING) {
    this->attempts = 0;
  ...
  this->status = INTERWEBS_STATUS_MQTT_SUBSCRIBING;
  ...
  this->status = INTERWEBS_STATUS_READING_SUBACK_PACKET;
  this->readFullPacketUntilComplete();
  return true;
}
...
bool Interwebs::mqttAnnounce(void) {
  ...
  this->status = INTERWEBS_STATUS_MQTT_ANNOUNCED;
  return true;
}

bool Interwebs::sendDiscoveries(void) {
...
bool Interwebs::verifyConnection(void) {
  ...
  this->status = INTERWEBS_STATUS_READING_PING_PACKET;
  return true;
}

bool Interwebs::wifiIsConnected(void) {
  return (int)this->status >= (int)INTERWEBS_STATUS_WIFI_CONNECTED;
}

bool Interwebs::mqttIsConnected(void) {
  return (int)this->status >= (int)INTERWEBS_STATUS_OKAY;
}

bool Interwebs::mqttIsActive(void) {
  return (int)this->status > (int)INTERWEBS_STATUS_OKAY;
}
...
bool Interwebs::setWill(String topic, String payload, uint8_t qos, bool retain) {
...
void Interwebs::addDiscovery(String topic, String payload, uint8_t qos, bool retain) {
  if (this->mqttIsConnected()) {
...
void Interwebs::mqttSendMessage(String topic, String payload, bool retain, uint8_t qos) {
  if (!this->wifiClient->connected()) {
    this->status = INTERWEBS_STATUS_MQTT_OFFLINE;
    return;
  }
...
bool Interwebs::mqttPublish(String topic, String payload, bool retain, uint8_t qos) {
  if (this->status != INTERWEBS_STATUS_MQTT_PUBLISHED) {
    ...
    this->status = INTERWEBS_STATUS_READING_PUBACK_PACKET;
    this->readFullPacketUntilComplete();
    return true;
  }
  else if (this->status == INTERWEBS_STATUS_MQTT_PUBLISHED && qos > 0) {
...
void Interwebs::readFullPacketUntilComplete(void) {
      ...
      this->full_packet_len = (this->read_packet_pbuf - this->read_packet_buf) + this->read_packet_len;
      {
        ...
        switch (packetType) {
          case MQTT_CTRL_CONNECTACK:
            if (this->status == INTERWEBS_STATUS_READING_CONACK_PACKET) {
              this->status = INTERWEBS_STATUS_MQTT_CONNECTED_TO_BROKER;
            }
            break;
          case MQTT_CTRL_PUBLISH:
            this->status = INTERWEBS_STATUS_SUBSCRIPTION_PACKET_READ;
            break;
          case MQTT_CTRL_SUBACK:
            this->status = INTERWEBS_STATUS_MQTT_SUBSCRIBING;
            break;
          case MQTT_CTRL_PUBACK:
            if (this->status == INTERWEBS_STATUS_READING_PUBACK_PACKET) {
              this->status = INTERWEBS_STATUS_MQTT_PUBLISHED;
            }
            break;
          case MQTT_CTRL_PINGRESP:
            if (this->status == INTERWEBS_STATUS_READING_PING_PACKET) {
              this->status = INTERWEBS_STATUS_OKAY;
            }
            break;
...
bool Interwebs::readPacketUntilComplete(void) {
  // Check we haven't timed out.
...
bool Interwebs::handleSubscriptionPacket(uint16_t len) {
...
uint8_t Interwebs::connectPacket(void) {
...
uint16_t Interwebs::publishPacket(const char *topic, uint8_t *data, uint16_t bLen, uint8_t qos, bool retain) {
  uint8_t *p = this->buffer;
  uint16_t len = 0;
...
uint8_t Interwebs::subscribePacket(const char *topic, uint8_t qos) {
  uint8_t *p = this->buffer;
  uint16_t len;
